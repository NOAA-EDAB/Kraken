...
PredationModuleTypeII::PredationModuleTypeII(Parameters *paramObj, ModelFormParameters *modelFormObj) {
...
double PredationModuleTypeII::getModelComponentValue(Parameters* paramObj, int speciesIndex, int timestep) const {
    ...
    for (int j = 0; j < numGroups; j++) {    // Subscript for predator
        ...
        lossBySpecies = -1.0 * (Bi * Bj * Aij * Sij) / denominator;
...
void PredationModuleTypeII::updateOutputLossesByGroupMatrix(Parameters* paramObj, int affectedSpeciesIndex, int affectingSpeciesIndex, int timeStep, double lossValue) const {

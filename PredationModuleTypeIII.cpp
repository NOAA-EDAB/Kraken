...
PredationModuleTypeIII::PredationModuleTypeIII(Parameters *paramObj, ModelFormParameters *modelFormObj) {

    m_ComponentName = "Type III Predation";
...
double PredationModuleTypeIII::getModelComponentValue(Parameters* paramObj, int speciesIndex, int timestep) const {
    ...
    double bCoef_ij = 0.0;                      // b Coefficient with main prey
    double bCoef_kj = 0.0;                      // b Coefficient with other prey
    ...
    for (int j = 0; j < numGroups; j++) {    // Subscript for predator
        ...
        // Get main prey's b_coef with the predator
        bCoef_ij = paramObj->getbCoef(i,j);
...
//            updateOutputLossesByGroupMatrix(paramObj, speciesIndex, j, lossBySpecies);
        ...
        lossToPredation += -1.0 * (pow(Bi,bCoef_ij) * Bj * Aij * Sij) / denominator;
    }
    return lossToPredation;
}


void PredationModuleTypeIII::updateOutputLossesByGroupMatrix(Parameters* paramObj, int affectedSpeciesIndex, int affectingSpeciesIndex, int timeStep, double lossValue) const {
